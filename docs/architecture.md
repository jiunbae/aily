...
**Runtime:** Runs as a persistent process (typically in a tmux session `agent-bridge` on host-mini). Uses aiohttp (not urllib, which gets 403 from Discord API).
...
| Host | Access | Purpose |
|------|--------|---------|
| host-mini | `ssh host-mini` (host.internal) | OrbStack host, primary dev machine |
| host-mbp | `ssh host-mbp` (100.64.0.2 via Tailscale) | MacBook Pro, secondary dev |
...
| Command | Action |
|---------|--------|
| `!new <name> [host]` | Create tmux session (default: host-mini) + Discord thread |
...
- [ ] Ensure SSH access from bridge host to tmux hosts (`host-mini`, `host-mbp`)
