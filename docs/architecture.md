# Messaging ↔ Claude Code Bidirectional Bridge

A system that connects Discord and Slack to Claude Code sessions running in tmux, enabling remote task management and automatic completion notifications.
...
│ Discord/  │◀────│ slack-bridge  │◀────│         │◀────│ (Claude Code)    │
│  Slack    │     │ (Python/WS)   │     └─────────┘     └──────────────────┘
└──────────┘     └───────────────┘                      │
                                                        │ Notification Hook
                                                        ▼
...
1. **User → Claude Code** (via bridges): User sends a message in a `[agent]` thread → bridge deterministically forwards it to the corresponding tmux session via SSH + `tmux send-keys`
2. **Claude Code → User** (via hooks): Claude Code finishes a task → notification hook posts via `post.sh` dispatcher to all configured platforms
...
### 5. Session Lifecycle (`thread-sync.sh` + tmux hooks)

**Trigger:** tmux `session-created` and `session-closed` global hooks

**What it does:**
- `session-created`: Finds or creates threads on all configured platforms, posts "Session started" message
- `session-closed`: Posts "Session closed" message, archives threads
- Skips infrastructure sessions (e.g., `agent-bridge`, `slack-bridge`)
- Forks to background immediately so tmux is not blocked
- Sources both `discord-lib.sh` and `slack-lib.sh` based on available tokens
...
### 6. Bridge Commands (agent-bridge.py / slack-bridge.py)

**Trigger:** Messages starting with `!` in the workspace channel or any thread

| Command | Action |
|---------|--------|
| `!new <name> [host]` | Create tmux session (default: first SSH host) + thread |
| `!kill <name>` | Kill tmux session + archive thread |
| `!sessions` | List all sessions across hosts with thread sync status |

Both bridges (`agent-bridge.py` for Discord, `slack-bridge.py` for Slack) support the same commands.

### 7. aily CLI

Command-line tool for managing thread sync from the terminal.
...
Auto-detects the current tmux session name when no argument is provided (via `TMUX_PANE`). Operates on all configured platforms.

### 8. Multi-platform Dispatcher (`post.sh`)

All notification hooks route through `post.sh` which auto-detects enabled platforms:

```
notify-claude.sh / notify-gemini.sh / notify-codex.py
    → post.sh (reads NOTIFY_PLATFORMS or auto-detects from tokens)
        → discord-post.sh (if Discord configured)
...
When a new thread is created (via tmux hooks, `aily start`, or `!new`), a welcome message is automatically posted with available commands. Implemented in `discord-lib.sh`, `slack-lib.sh`, `agent-bridge.py`, and `slack-bridge.py`.
...
│   ├── discord-thread-sync.sh     # Backward compat wrapper
...
│   ├── format-question.py         # Question formatter
...
├── .env.example
├── requirements.txt       # Python deps (aiohttp, slack-sdk)
├── .gitignore
├── .venv/                 # Python venv
├── install.sh
└── README.md
