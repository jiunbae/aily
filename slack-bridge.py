...
class SlackPlatform(PlatformBridge):
    ...
    def __init__(self, web_client: AsyncWebClient, state: BridgeState):
    ...
    async def find_thread(self, thread_name: str) -> str | None:
        ...
        try:
            result = await self._client.conversations_history(
                channel=self._state.channel_id, limit=200
            )
    ...
    async def create_thread(self, thread_name: str, starter_msg: str) -> str | None:
        ...
        try:
            result = await self._client.chat_postMessage(
                channel=self._state.channel_id, text=starter_msg
            )
            parent_ts = result.get("ts")
            if not parent_ts:
                return None

            session_name = self._state.parse_thread_name(thread_name) or thread_name
            welcome = (
            ...
            await self._client.chat_postMessage(
                channel=self._state.channel_id, thread_ts=parent_ts, text=welcome
            )
            return parent_ts
        except Exception as e:
    ...
    async def archive_thread(self, thread_id: str) -> None:
        ...
        try:
            ...
            await self._client.reactions_add(
                channel=self._state.channel_id, timestamp=thread_id, name="lock"
            )
        except Exception as e:
    ...
    async def delete_thread(self, thread_id: str) -> None:
        ...
        try:
            await self._client.chat_delete(
                channel=self._state.channel_id, ts=thread_id
            )
        except Exception as e:
    ...
    async def get_active_thread_sessions(self) -> set[str]:
        ...
        try:
            ...
            for msg in result.get("messages", []):
                text = msg.get("text", "")
                first_line = text.split("\n")[0].strip()
                session = self._state.parse_thread_name(first_line)
                if session:
    ...
    async def get_thread_session(self, channel: str, thread_ts: str) -> str | None:
        ...
        session_name = self._state.parse_thread_name(first_line)
        if session_name:
...
async def handle_socket_event(
    ...
    if text.startswith("!"):
        # Check if it's a shortcut in a thread — route to core.handle_message
...
async def main():
    env, env_path = BridgeCore.load_env_file()
    ...
    state = BridgeCore.load_common_config(env, channel_id=channel_id)

    # Create Slack platform + core
    web_client = AsyncWebClient(token=bot_token)
    platform = SlackPlatform(web_client, state)
    ...
    # Print startup info
    core.print_startup_info("slack-bridge", extra={
        "Bot": f"{auth.get('user', '')} ({bot_user_id})",
        "Channel": channel_id,
    })

    # Initialize dashboard HTTP session
    await core.init_dashboard_session()
